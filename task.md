...
目标：在 `players/<你的名字>.py` 里实现一个玩家，让 Mario 从 RAM 状态出发通过尽可能多的关卡。没有像素，没有截图，只有结构化状态。

## 规则

- 只允许新增或修改 `players/<你的名字>.py` 这一个文件，直接在仓库 main 上做。`arena/` 目录、动作词表、harness 参数一律不许改，改了成绩作废。
